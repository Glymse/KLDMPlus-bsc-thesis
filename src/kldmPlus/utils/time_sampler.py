...
class AdaptiveReinforcePaperTimeSampler:
    """
    Paper-style adaptive timestep sampler with REINFORCE and feature selection.

    Mirrors the algorithm in Adaptive Non-Uniform Timestep Sampling by
    optimizing a Beta-policy sampler to maximize the expected loss reduction
    on a selected subset of probe timesteps.
    """
    ...
    def __init__(
        ...
        *,
        ...
        entropy_coef: float = 1e-2,
        policy_update_every: int = 40,
        policy_warmup_steps: int = 0,
        reward_candidate_times: int = 13,
        ...
        if not probe_times:
            raise ValueError("At least one reward probe time is required.")
        self.reward_probe_times = torch.tensor(probe_times, dtype=torch.float64)
        self.reward_active_times = min(int(self.reward_active_times), len(probe_times))
        if self.reward_active_times <= 0:
            raise ValueError("reward_active_times must be positive.")
        ...
        self._target_history: list[torch.Tensor] = []
        self._pending_policy_update = False
        self._probe_cache: dict[str, Any] | None = None
    ...
    def _default_probe_indices(self) -> torch.Tensor:
        ...
        if num_selected <= 0:
            return torch.zeros(0, dtype=torch.long)
        return torch.arange(num_selected, dtype=torch.long)

    def _append_feature_history(self, delta_vector: torch.Tensor, delta_sum: torch.Tensor) -> None:
        self._feature_history.append(delta_vector.detach().to(device="cpu", dtype=torch.float64))
        self._target_history.append(delta_sum.detach().to(device="cpu", dtype=torch.float64))
        if len(self._feature_history) > self.reward_history_size:
            del self._feature_history[:-self.reward_history_size]
        if len(self._target_history) > self.reward_history_size:
            del self._target_history[:-self.reward_history_size]

    def _select_probe_indices(self) -> torch.Tensor:
        if self.reward_active_times >= self.reward_probe_times.numel():
            return torch.arange(self.reward_probe_times.numel(), dtype=torch.long)
        if len(self._feature_history) < self.feature_selection_min_history:
            return self._default_probe_indices()

        history = torch.stack(self._feature_history, dim=0)
        target = torch.stack(self._target_history, dim=0).reshape(-1)
    ...
    def load_state_dict(self, state_dict: dict[str, Any] | None) -> None:
        ...
        if selected_probe_indices is not None:
            self.selected_probe_indices = selected_probe_indices.to(dtype=torch.long, device="cpu")
        feature_history = state_dict.get("feature_history")
        if feature_history is not None:
            self._feature_history = [
                item.to(device="cpu", dtype=torch.float64)
                for item in feature_history
            ]
        target_history = state_dict.get("target_history")
    ...
    def diagnostics(self) -> dict[str, float]:
        if int(self.selected_probe_indices.numel()) == 0:
            selected_times = self.reward_probe_times[:1].to(dtype=torch.float64)
        else:
            selected_times = self.reward_probe_times[self.selected_probe_indices].to(dtype=torch.float64)
        return {
            "time_sampler/reward_mean": float(self.last_reward_mean),
